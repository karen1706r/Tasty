...
export class ShowChefComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Configurar polling para recargar los pedidos cada 5 segundos
    this.pollingSubscription = interval(2000).subscribe(() => {
      this.cargarPedidos(); // Actualizar pedidos cada 5 segundos
    });
  }
  ...
  cargarPedidos(): void {
    this.restauranteService.getPedidosChef().subscribe({
      next: (data) => {
        // Filtrar por estados_p y por id_categoria = 9
